...
function sendmail($toemail, $subject, $message, $from='') {
	...
	if($mail['mailsend'] == 3) {
		...
		//$email_from = $from == '' ? '=?'.$_SC['charset'].'?B?'.base64_encode($_SC['site_name'])."?= <".$_SCONFIG['adminemail'].">" : (preg_match('/^(.+?) \<(.+?)\>$/',$from, $mats) ? '=?'.$_SC['charset'].'?B?'.base64_encode($mats[1])."?= <$mats[2]>" : $from);
	...
	$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: SYLAI ".X_VER."{$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=$_SC[charset]{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
	...
	if($mail['mailsend'] == 1) {
		...
		fputs($fp, ($mail['auth'] ? 'EHLO' : 'HELO')." sylai\r\n");
		$lastmessage = fgets($fp, 512);
