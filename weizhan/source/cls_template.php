...
class cls_template
{
    ...
    {
        $name = $this->compile_dir . '/' . basename($filename) . '_' . $this->compile_id . '.php';
        if ($this->_expires)
        {
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        $tmp_dir   = 'themes/' . $_SCONFIG['template'] . '/'; // 模板所在路径
